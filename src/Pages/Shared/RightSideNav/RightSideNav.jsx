...
const RightSideNav = (props) => {
  ...
  const handleLoginWithGoogle = (e) => {
    ...
    signInWithGoogle().then((res) =>
      console.log(res.user).catch((error) => console.log(error))
    );
  };
    ...
    <div>
      <div className="flex flex-col p-4 w-full space-y-4">
        ...
        <button
          ...
          className="btn btn-outline btn-accent m-2 text-sm"
        >
          <FaGoogle /> Login with Google
        </button>
        <button onClick={handleLoginWithGithub} className="btn btn-outline text-black m-2 textarea-sm">
